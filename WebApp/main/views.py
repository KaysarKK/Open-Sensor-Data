...
def detail_city_inside(request, city_id):
	try:
		city = City.objects.get(pk = city_id)
		device = Device.objects.get(pk = city)
	except City.DoesNotExist:
		raise Http404("There is no such Device")
	return render(request, 'detail2.html', {'device' : device})
